...
function isValidAdvisorModel(modelID: string): boolean {
  return stripProviderPrefix(modelID).includes(DEFAULT_ADVISOR_MODEL)
}
...
export function validateAnthropicAdvisorPair(
  ...
  if (!isValidExecutorModel(executorModel.api.id) && !isValidExecutorModel(executorModel.id)) {
    throw new Error(
      ...
      + "Supported executors currently include Claude Haiku 4.5, Claude Sonnet 4.6, Claude Opus 4.6, and Claude Opus 4.7.",
    )
  }
}
