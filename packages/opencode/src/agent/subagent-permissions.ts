...
export function deriveSubagentSessionPermission(input: {
  ...
  const canTask = subagentAllowsPermission(input.subagent, "task")
  const canTodo = subagentAllowsPermission(input.subagent, "todowrite")
...
export function subagentAllowsPermission(subagent: Agent.Info, permission: string) {
  return subagent.permission.some((rule) => {
    if (rule.action !== "allow") return false
    if (rule.permission !== permission) return false
    return Permission.evaluate(permission, rule.pattern, subagent.permission).action === "allow"
  })
}
