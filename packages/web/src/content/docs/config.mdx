...
If you are smoke testing the local Claude Code provider, start with Haiku:
...
For day-to-day Claude Code work inside OpenCode, a cleaner runtime usually looks like this:
...
For `claude-code`, additional provider options include `settingSources`, `disableSlashCommands`, `strictMcpConfig`, `pluginDir`, `mcpConfig`, `addDir`, `tools`, and `permissionMode`. `bare` is also supported, but it disables Claude App OAuth and is intended for API-key flows.
